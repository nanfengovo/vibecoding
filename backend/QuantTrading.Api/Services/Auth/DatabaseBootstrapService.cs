...
public sealed class DatabaseBootstrapService
{
    ...
    private async Task EnsureSchemaAsync(CancellationToken cancellationToken)
    {
        if (IsPostgres())
        {
            foreach (var sql in BuildPostgresSchemaSql())
            {
                ...
                catch (Exception ex) when (IsOptionalVectorSql(sql))
                {
                    _logger.LogWarning(ex, "Skip optional pgvector SQL during bootstrap.");
                }
            }
    ...
    private static bool IsOptionalVectorSql(string sql)
    {
        var normalized = (sql ?? string.Empty).ToLowerInvariant();
        return normalized.Contains("create extension if not exists vector")
            || normalized.Contains("knowledgechunkembeddings")
            || normalized.Contains(" embedding\" vector(");
    }
    ...
    private static IEnumerable<string> BuildPostgresSchemaSql()
    {
        ...
        // Keep extension creation idempotent; index creation is intentionally excluded
        // from app startup and must be executed by an independent DB job.
        yield return """CREATE EXTENSION IF NOT EXISTS vector""";
...
""";
        yield return """CREATE INDEX IF NOT EXISTS "IX_AiToolTraceAudits_UserId_CreatedAt" ON "AiToolTraceAudits" ("UserId", "CreatedAt")""";
        yield return """CREATE INDEX IF NOT EXISTS "IX_AiToolTraceAudits_SessionId_CreatedAt" ON "AiToolTraceAudits" ("SessionId", "CreatedAt")""";
...
""";
        yield return """CREATE UNIQUE INDEX IF NOT EXISTS "IX_AiResponseCaches_CacheKey" ON "AiResponseCaches" ("CacheKey")""";
        yield return """CREATE INDEX IF NOT EXISTS "IX_AiResponseCaches_ExpiresAt" ON "AiResponseCaches" ("ExpiresAt")""";
...
""";
        yield return """CREATE UNIQUE INDEX IF NOT EXISTS "IX_KnowledgeChunkEmbeddings_ChunkId_Model" ON "KnowledgeChunkEmbeddings" ("ChunkId", "Model")""";
...
""";
        ...
        yield return """
IF OBJECT_ID(N'[AiToolTraceAudits]', N'U') IS NULL
CREATE TABLE [AiToolTraceAudits] (
    [Id] bigint IDENTITY(1,1) NOT NULL CONSTRAINT [PK_AiToolTraceAudits] PRIMARY KEY,
    [UserId] int NOT NULL,
    [SessionId] bigint NULL,
    [Orchestrator] nvarchar(40) NOT NULL DEFAULT N'',
    [ExecutionMode] nvarchar(20) NOT NULL DEFAULT N'',
    [ToolName] nvarchar(120) NOT NULL DEFAULT N'',
    [Source] nvarchar(60) NOT NULL DEFAULT N'',
    [Status] nvarchar(20) NOT NULL DEFAULT N'',
    [LatencyMs] int NOT NULL DEFAULT 0,
    [RequestDigest] nvarchar(128) NOT NULL DEFAULT N'',
    [ResponseDigest] nvarchar(128) NOT NULL DEFAULT N'',
    [RequestPreview] nvarchar(600) NOT NULL DEFAULT N'',
    [ResponsePreview] nvarchar(600) NOT NULL DEFAULT N'',
    [ErrorCode] nvarchar(80) NOT NULL DEFAULT N'',
    [CreatedAt] datetime2 NOT NULL
)
""";
        yield return """IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_AiToolTraceAudits_UserId_CreatedAt' AND object_id = OBJECT_ID(N'[AiToolTraceAudits]')) CREATE INDEX [IX_AiToolTraceAudits_UserId_CreatedAt] ON [AiToolTraceAudits] ([UserId], [CreatedAt])""";
        yield return """IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_AiToolTraceAudits_SessionId_CreatedAt' AND object_id = OBJECT_ID(N'[AiToolTraceAudits]')) CREATE INDEX [IX_AiToolTraceAudits_SessionId_CreatedAt] ON [AiToolTraceAudits] ([SessionId], [CreatedAt])""";

        yield return """
IF OBJECT_ID(N'[AiResponseCaches]', N'U') IS NULL
CREATE TABLE [AiResponseCaches] (
    [Id] bigint IDENTITY(1,1) NOT NULL CONSTRAINT [PK_AiResponseCaches] PRIMARY KEY,
    [CacheKey] nvarchar(200) NOT NULL,
    [Category] nvarchar(60) NOT NULL DEFAULT N'',
    [Payload] nvarchar(max) NOT NULL DEFAULT N'',
    [ExpiresAt] datetime2 NOT NULL,
    [CreatedAt] datetime2 NOT NULL
)
""";
        yield return """IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_AiResponseCaches_CacheKey' AND object_id = OBJECT_ID(N'[AiResponseCaches]')) CREATE UNIQUE INDEX [IX_AiResponseCaches_CacheKey] ON [AiResponseCaches] ([CacheKey])""";
        yield return """IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_AiResponseCaches_ExpiresAt' AND object_id = OBJECT_ID(N'[AiResponseCaches]')) CREATE INDEX [IX_AiResponseCaches_ExpiresAt] ON [AiResponseCaches] ([ExpiresAt])""";
...
IF OBJECT_ID(N'[KnowledgeChunkEmbeddings]', N'U') IS NULL
CREATE TABLE [KnowledgeChunkEmbeddings] ([Id] bigint IDENTITY(1,1) NOT NULL CONSTRAINT [PK_KnowledgeChunkEmbeddings] PRIMARY KEY, [KnowledgeBaseId] bigint NOT NULL, [DocumentId] bigint NOT NULL, [ChunkId] bigint NOT NULL, [UserId] int NOT NULL, [Model] nvarchar(120) NOT NULL DEFAULT N'', [Dimension] int NOT NULL DEFAULT 0, [Embedding] nvarchar(max) NOT NULL DEFAULT N'', [CreatedAt] datetime2 NOT NULL, [UpdatedAt] datetime2 NOT NULL);
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_KnowledgeChunkEmbeddings_ChunkId_Model' AND object_id = OBJECT_ID(N'[KnowledgeChunkEmbeddings]')) CREATE UNIQUE INDEX [IX_KnowledgeChunkEmbeddings_ChunkId_Model] ON [KnowledgeChunkEmbeddings] ([ChunkId], [Model]);
